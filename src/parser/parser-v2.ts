...
export const parseV2 = (tokens: Token[], options?: ParserOptions) => {
	...
	function processToken(tokens: Token[]): Node {
		...
		if (isTag(currentToken)) {
			...
			if (isOpeningTag(currentToken)) {
				const [enclosedTokens, remainingTokens] = grabTokensUntil(
					tokens.slice(1),
					(t: Token) => isClosingTag(t) && t[1].slice(1) === currentToken[1],
				);
				// console.log("currentTag", currentToken[1], "grabbed", enclosedTokens);
