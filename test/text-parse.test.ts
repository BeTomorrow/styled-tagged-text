...
describe("Inline styled text parse test", () => {
	...
	test("Starting with tag", () => {
		const input = "[b]Foo [a]Bar[/a][/b]";
		const ast = toAst(input);
		const output = [{ tag: "b", content: ["Foo ", { tag: "a", content: "Bar" }] }];
		expect(ast).toEqual(output);
	});
	...
	// 	const ast = toAst(input);
	// 	const output = [{ tag: "b", content: ["Foo ", [{ tag: "a", content: "Bar" }, " "], { tag: "c", content: { tag: "a", content: "Baz" } }] }];
	// 	expect(ast).toEqual(output);
	// });
});
